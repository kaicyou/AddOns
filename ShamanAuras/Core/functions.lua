...
function Auras:UpdateTalents(isTalentChange)
	...
	if (spec == 1) then -- Elemental
		...
		rowObj = {
			[1] = SSA.LightningRod,
			[2] = SSA.ElementalFocus,
			[3] = SSA.PowerOfMaelstrom,
			[4] = SSA.Concordance1,
		}
		
		rowList = {
			[1] = db.auras[1].LightningRod and select(4,GetTalentInfo(7,2,1)),
			[2] = db.auras[1].ElementalFocus and IsSpellKnown(16164),
			[3] = db.auras[1].PowerOfMaelstrom and (rank or 0) >= 12,
			[4] = db.auras[1].Concordance1 and (rank or 0) >= 52,
		}
	...
	elseif (spec == 2) then -- Enhancement
		...
		rowList = {
			...
			[6] = db.auras[2].FeralSpirit and IsSpellKnown(51533),
		}
		...
		rowList = {
			...
			[5] = db.auras[2].CleanseSpirit2 and IsSpellKnown(51886),
		}
		...
		rowList = {
			[1] = db.auras[2].SpiritWalk and IsSpellKnown(58875),
			[2] = db.auras[2].AstralShift2 and IsSpellKnown(108271),
		...
		rowList = {
			...
			[4] = db.auras[3].HealingRain and IsSpellKnown(73920),
		...
		rowList = {
			...
			[2] = db.auras[3].HealingTideTotem and IsSpellKnown(108280),
			[3] = db.auras[3].SpiritLinkTotem and IsSpellKnown(98008),
